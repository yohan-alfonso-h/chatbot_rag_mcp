...
type ServiceErrorCode = "AUTH" | "RATE_LIMIT" | "MODEL" | "EMPTY_RESPONSE" | "UNKNOWN";
...
class GeminiService {
  ...
  constructor(apiKey:string, moduleName:string) {
    this.apiKey = apiKey;
  ...
  async generateResponse(prompt: string): Promise<string> {
    try {
      ...
      if (!response.text) {
        const err = new Error("Gemini returned an empty response") as Error & { code?: ServiceErrorCode };
      ...
      console.error("Error generating response from Gemini API:", message);

      const wrapped = new Error(message || "Gemini request failed") as Error & { code?: ServiceErrorCode };
  ...
  async generateEmbedding(text: string | string[], taskType = "RETRIEVAL_QUERY") {
